...
public class HomeActivity extends BaseActivity
    ...
    private class MyTimerTask extends TimerTask {
        public void run() {
            String url_reply = "这里是URl.php?mod=space&do=notice&view=mypost&type=post" + (App.IS_SCHOOL_NET ? "" : "&mobile=2");
            String url_pm = "这里是URl.php?mod=space&do=pm&mobile=2";
    ...
    public void onBackPressed() {
        if (getFragmentManager().getBackStackEntryCount() > 0) {
            ...
            if ((System.currentTimeMillis() - mExitTime) > 1500) {
                Toast.makeText(this, "再按一次退出手机信果(｡･ω･｡)~~", Toast.LENGTH_SHORT).show();
