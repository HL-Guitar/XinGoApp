...
public class PostActivity extends BaseActivity
    ...
    private void startBlock(int position) {
        String url = "这里是URl.php?mod=topicadmin&action=banpost"
                + "&fid=" + fid
                + "&tid=" + tid
    ...
    private void startWarn(int position) {
        if (App.IS_SCHOOL_NET) {
            ...
            String url = "这里是URl.php?mod=topicadmin&action=warn&fid=" + fid
                    + "&tid=" + tid
    ...
    private void startClose() {
        String url = "";
        if (App.IS_SCHOOL_NET) {
            url = "这里是URl.php?mod=topicadmin&action=moderate&fid=" + fid + "&moderate[]=" + tid + "&handlekey=mods" +
                    "&infloat=yes&nopost=yes&from=" + tid + "&inajax=1";
        } else {
            url = "这里是URl.php?mod=topicadmin&action=moderate&fid=" + fid
    ...
    private void startDelete(int position) {
        if (App.IS_SCHOOL_NET) {
            ...
            if (datas.get(position).type == SingleType.CONTENT) {
                ...
                //删除评论
                url = "这里是URl.php?mod=topicadmin&action=delpost&fid=" + fid
