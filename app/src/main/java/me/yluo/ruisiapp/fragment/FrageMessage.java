...
public class FrageMessage extends BaseLazyFragment implements LoadMoreListener.OnLoadMoreListener {
    ...
    private void getData(boolean needRefresh) {
        ...
        String url;
        if (index == 0) { //reply
            url = "这里是URlhome.php?mod=space&do=notice&mobile=2&page=" + currentPage;
        } else if (index == 1) { //pm
            url = "这里是URlhome.php?mod=space&do=pm&mobile=2&page=" + currentPage;
        } else { //@wo
            url = "这里是URlhome.php?mod=space&do=notice&view=mypost&type=at&mobile=2&page=" + currentPage;
        }
