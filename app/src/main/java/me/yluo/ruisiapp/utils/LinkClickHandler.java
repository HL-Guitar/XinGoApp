...
public class LinkClickHandler {
    ...
    public static void handleClick(final Context context, String url) {
        ...
        } else if (url.contains("这里是URlforum.php?mod=viewthread&tid=") || url.contains("forum.php?mod=redirect&goto=findpost")) { // 帖子
            PostActivity.open(context, url, null);
        } else if (url.contains("这里是URlhome.php?mod=space&uid=")) { // 用户
            String imageUrl = UrlUtils.getAvaterurlb(url);
            UserDetailActivity.open(context, "name", imageUrl, "");
        } else if (url.contains("这里是URlforum.php?mod=post&action=newthread")) { //发帖链接
            context.startActivity(new Intent(context, NewPostActivity.class));
        } else if (url.contains("这里是URlmember.php?mod=logging&action=login")) {//登陆
            LoginActivity.open(context);
        } else if (url.contains("这里是URlforum.php?mod=forumdisplay&fid=")) {
            int fid = GetId.getFroumFid(url);
            PostsActivity.open(context, fid, "分区帖子");
        } else if (url.startsWith("这里是URlforum.php?mod=post&action=reply")) { //回复
            if (context instanceof PostActivity) {
                PostActivity a = (PostActivity) context;
                a.showReplyKeyboard();
            }
        } else if (url.contains("这里是URlforum.php?mod=attachment")) {
